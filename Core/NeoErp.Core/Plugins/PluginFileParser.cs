...
namespace NeoErp.Core.Plugins
{
    ...
    public static class PluginFileParser 
    {
        ...
        //public static string CheckInstallsModule() {
        //    ModuleService ms = new ModuleService(EngineContext.Current.Resolve<IDbContext>());
        //    return ms.CheckModuleExists();
        ...
        public static string InsertModuleToDB(PluginDescriptor pluginDescription)
        {
            ModuleService ms = new ModuleService(EngineContext.Current.Resolve<IDbContext>(), EngineContext.Current.Resolve<NeoErpCoreEntity>());
            return ms.SaveModuleDetail(pluginDescription); 
        }
        public static int UninstallModuleFormDB(PluginDescriptor pluginDescription)
        {
            ModuleService ms = new ModuleService(EngineContext.Current.Resolve<IDbContext>(), EngineContext.Current.Resolve<NeoErpCoreEntity>());
            return ms.DeleteModuleByUninstall(pluginDescription);
        }
        public static PluginDescriptor ParsePluginDescriptionFile(string filePath)
        {
            ...
            //NeoErpCommerce 2.00 didn't have 'SupportedVersions' parameter
        ...
        public static IList<string> ParseInstalledPluginsDataBase()
        {
         //   NeoErp.Core.Models.NeoCoreEntity _db = new Models.NeoCoreEntity();
            var listString = new List<string> {"",""};
            return listString;
           //return _db.Modules.ToList().Select(x => x.ModuleID).ToList();
        }
    }
}
