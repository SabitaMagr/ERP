...
var urltest = window.location.protocol + "//" + window.location.host + "/api/LcReport/OptimizedPendingLcReport";
...
function BindGrid(urltest) {
    ...
    var dataSource = new kendo.data.DataSource({
        ...
        serverPaging: true,
        schema: {
            //data:"OptimizedPendingLcModel",
            aggregates: "AggregationResult",
            model: {
                fields: {
                    LcWeekNumber: { type: "string" },
                    LcNumber: { type: "string" },
                    DocumentOrderNumber: { type: "string" },
                    CommercialInvoice: { type: "string" },
                    ItemName: { type: "string" },
                    OrderQunatity: { type: "number" },
                    PendingPurchaseOrder: { type: "number" },
                    MIT: { type: "string" },
                    Due: { type: "number" },
                    ATPORT: { type: "string" },
                    ATA: { type: "string" },
                    AwbRecievedDate: { type: "string" }

                }
            }
        },
        aggregate: [
            { field: "OrderQunatity", aggregate: "sum" },
            { field: "PendingPurchaseOrder", aggregate: "sum" },
        ],
        pageSize: 100
    });
    var grid = $("#grid").kendoGrid({
        ...
        columns: [
            ...
            {
                field: "AwbRecievedDate",
                title: "AWB Recieved Date",
