distributionModule.controller('Scheme', function ($scope, $http, $routeParams, $filter) {
    debugger;
    ...
    $scope.schemeCreate = function (isValid) {
        debugger;
        /*$(".loader").show();*/
        ...
        if (schemeType == "GIFT") {
            ...
            //    return;
            //}
            var discount = $scope.DiscountAmount;
            var discountType = $("#DiscountType").data("kendo-drop-down-list").value();
            //if (discountType == "") {
            //    displayPopupNotification("Discount Type Empty", "warning");
            //    return;
            //}
            //var schemeData = {
            //    Action: actionname,
            ...
            document.getElementById("CreatePanel").style.opacity = " 0.4";            
            $(".loader").show();
            $http({
                method: 'POST',
                data: schemeData,
                ...
                if (response.data == "Success") {
                    $scope.createPanel = false;
                    $("#grid").data("kendoGrid").dataSource.read();
                    document.getElementById("CreatePanel").style.pointerEvents = "";
                    document.getElementById("CreatePanel").style.opacity = "";
                    displayPopupNotification("Scheme saved successfully", "success");
                    $scope.showOtherOptions("");
    ...
    $scope.grid = {
        ...
        columnShow: function (e) {
            if ($(".k-widget.k-reset.k-header.k-menu.k-menu-vertical").is(":visible") && checkboxItem != "")
                SaveReportSetting('SchemeSetup', 'grid');
        },
        ...
        columns: [{
            ...
            {
                title: "Action",
                template: " # if(Status == null){ #<a class='fa fa-edit editAction' ng-click='updateScheme($event)' title='Edit'></a>&nbsp &nbsp#}#<a class='fa fa-trash-o deleteAction' ng-click='deleteScheme($event)' title='delete'></a> ",
                width: "8%"
            }
        ]
    };
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    template:' <textarea  rows="4" cols="35" disabled>#= ItemName #</textarea> ',
                    title: "Items",
                    width: "100%"
                },
                {
    ...
    //$scope.giftItems = function (dataItem) {
    //    debugger;
    //    var id = dataItem.SchemeID;
    //    return {
    //        dataSource: {
    ...
    $scope.updateScheme = function (evt) {
        debugger;
        ...
        //$("#areaMultiSelect").data("kendoMultiSelect").value([item.AreaCode]);
        ...
        if (item.SP_CODE != null) {
            ...
            var selectedArea = _.pluck(_.filter(EmpMultiSelect.dataSource.data(), function (x) {
                ...
                ////dist select
                //var selectedDistributor = _.pluck(_.filter(multiSelect.dataSource.data(), function (x) {
                //    return x.AREA_CODE == item.AreaCode;
                //}), "DISTRIBUTOR_CODE");
