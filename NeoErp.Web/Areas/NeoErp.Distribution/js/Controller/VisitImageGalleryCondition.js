distributionModule.controller('VisitImageGalleryConditionCtrl', function ($scope, DistSetupService, $routeParams) {
    ...
    $scope.applyAdvanceFilter = function () {
        ...
        var getdata = DistSetupService.GetVisiterListCondition(dateFilter1,data);
        getdata.then(function (response) {
            ...
            $.each(response.data, function (i, v) {
                if (v.CATEGORY_EDESC == "Reseller") {
                    v["url"] = window.location.protocol + "//" + window.location.host + "/Areas/NeoErp.Distribution/Images/ResellerImages/";
                }
                else {
                    v["url"] = window.location.protocol + "//" + window.location.host + "/Areas/NeoErp.Distribution/Images/EntityImages/";
                }
            });
