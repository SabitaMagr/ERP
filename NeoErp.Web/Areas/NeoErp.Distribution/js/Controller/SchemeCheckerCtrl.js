distributionModule.controller('SchemeCheckerCtrl', function ($scope, $http, $routeParams, $filter) {
    debugger;
    ...
    $scope.schemeReject = function (isValid) {
        ...
        $http({
            method: 'POST',
            url: window.location.protocol + "//" + window.location.host + "/api/Setup/ApproveScheme?SchemeId=" + $scope.SchemeID + "&Action=CR",
        }).then(function successCallback(response) {
            debugger;
    ...
    $scope.schemeApprove = function (isValid) {
        ...
        if (actionname == "Approve") {
            $http({
                method: 'POST',
                url: window.location.protocol + "//" + window.location.host + "/api/Setup/ApproveScheme?SchemeId=" + $scope.SchemeID + "&Action=" + "C",
            }).then(function successCallback(response) {
                debugger;
                if (response.data == true) {
                    $scope.createPanel = false;
                    ...
                    displayPopupNotification("Scheme checked successfully", "success");
                    $scope.showOtherOptions("");
    ...
    $scope.grid = {
        ...
        {
            title: "Action",
            template: " # if(CheckedStatus != 'N' && CheckedStatus != 'Y'){ #<a class='fa fa-edit editAction' ng-click='editScheme($event)' title='Edit'></a>&nbsp &nbsp#}#<a class='fa fa-trash-o deleteAction' ng-click='deleteScheme($event)' title='delete'></a> ",
            width: "8%"
        }
        ]
    };
