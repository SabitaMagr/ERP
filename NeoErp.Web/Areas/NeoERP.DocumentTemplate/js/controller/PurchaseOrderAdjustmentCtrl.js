DTModule.controller('PurchaseOrderAdjustmentCtrl', function ($scope, PurchaseOrderAdjustmentService, $filter) {

    $scope.TableName = "IP_PURCHASE_ORDER";

    $scope.FormName = "Purchase Order Adjustment";
    ...
    $scope.selectedOrderDoc = null;

    $scope.OrderFilter = "CAID";
    ...
    $scope.GetIndentDocument = function () {
        var indentDocument = PurchaseOrderAdjustmentService.getIndentDocument($scope.TableName);
        indentDocument.then(function (res) {
            $scope.orderDocument = res.data;
        });
    };
    ...
    $("#pOrderAdjustmentGrid").kendoGrid({
        ...
        columns: [
            ...
            { field: "SUPPLIER_EDESC", title: "Supplier Name", width: 120 },
    ...
    $scope.findNewIndentAdjustment = function () {
        ...
        console.log("scope.selected Doc=====================>>>" + $scope.selectedOrderDoc);
        var fromDate = $('#FromDateVoucher').val();
        var toDate = $('#ToDateVoucher').val();
        if (!$scope.selectedOrderDoc) {
            ...
            $scope.param = {
                "Document": $scope.selectedOrderDoc.FORM_CODE,
                "FromDate": fromDate,
                "ToDate": toDate,
                "IndentFilter": $scope.IndentFilter
            };

            var pOrderAdjustmentGrid = $("#pOrderAdjustmentGrid").data("kendoGrid");
            pOrderAdjustmentGrid.dataSource.transport.options.read.url = "/api/PurchaseOrderIndentAdjustmentApi/GetAllPurchaseOrderAdjustment?document=" + $scope.selectedOrderDoc.FORM_CODE + "&fromDate=" + fromDate + "&toDate=" + toDate + "&indentFilter=" + $scope.IndentFilter;
            pOrderAdjustmentGrid.dataSource.read();
        }
    };

    $scope.SetIndentDataToGrid = function (data) {
        var orderAdjustment = $("#pOrderAdjustmentGrid").data("kendoGrid");
        orderAdjustment.dataSource.transport.options.read.data = data;
        orderAdjustment.dataSource.read();
    };

    $scope.FilterOrder = function (value) {
        $scope.OrderFilter = value;
        console.log("Indent Filter =================" + $scope.OrderFilter);
    };

    $scope.saveOrderAdjustment = function () {
        ...
        var saveResponse = PurchaseOrderAdjustmentService.SaveOrderAdjustment($scope.checkedItemToSave);
...
DTModule.service('PurchaseOrderAdjustmentService', function ($http) {
    ...
    this.SaveOrderAdjustment = function (parameter) {

        var saveRes = $http({
            method: "POST",
            url: "/api/PurchaseOrderIndentAdjustmentApi/SaveOrderAdjustment",
