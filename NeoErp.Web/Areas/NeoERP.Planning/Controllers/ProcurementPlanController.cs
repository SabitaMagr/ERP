...
namespace NeoERP.Planning.Controllers
{
    public class ProcurementPlanController : Controller
    {
        ...
        public IPlan _plan;
        IProcurementPlanRepo _ProcurementPlanRepo;
        public ProcurementPlanController(IPlanSetup iPlanSetup, IProcurementPlanRepo ProcurementPlanRepo, IPlan PlanRepo)
        {
        ...
        public ActionResult FavProcurementPlanReport()
        {
            return PartialView();
        }
        public ActionResult CreateProcurementPlan(string plancode)
        {
            ...
            string employeCode = this._plan.getUsersEmployeeCode();
            ViewBag.EmployeeCode = employeCode;
            var model = this._iPlanSetup.GetPreferenceSetup("PROCUREMENT_PLAN");
            return PartialView(model);
        }
        public ActionResult ProcureFromMaterial(string plancode)
        {
            ViewBag.PlanCode = plancode;
            ViewBag.ProductSelectionLimit = System.Configuration.ConfigurationManager.AppSettings.Get("ProductSelectionLimit");
            string employeCode = this._plan.getUsersEmployeeCode();
        ...
        public ActionResult ProcurementPlanSetup(string planCode)
        {
            ViewBag.planCode = planCode;
        ...
        {
            return PartialView("~/Areas/NeoERP.Planning/Views/ProcurementPlan/_ProcurementPlanSetupTreeList.cshtml");
        }
        public ActionResult LoadMaterialPlanGridPartial()
        {
            return PartialView("~/Areas/NeoERP.Planning/Views/ProcurementPlan/_ProcureFromMaterialTreeList.cshtml");
        }
        public string SaveProcurementPlan(FormCollection fc)
        {
            ...
            ProcurementPlan sp = new ProcurementPlan()
            {
            ...
            if (parentArr.Count > 0)
                ...
                return result = this._ProcurementPlanRepo.SaveProcurementPlan(parentArr, sp);
            else
                return "error";
        }
        ...
        {
            var model = this._iPlanSetup.GetPreferenceSetup("PROCUREMENT_PLAN");
            PL_BRD_PRCMT_PLAN result = this._ProcurementPlanRepo.GetPlanDetailValueByPlanCode(planCode);
            ViewBag.planCode = planCode;
            return PartialView(model);
        }
        [HttpPost]
        public JsonResult CopyPlan(FormCollection formData)
        {
            ...
            {
                copied = this._ProcurementPlanRepo.cloneProcurementPlan(planCode, planName, customers, employees, branchs, divisions, remarks);
