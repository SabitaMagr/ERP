...
{
    public class LedgerBudgetPlanController : Controller
    {
        ILBAPlanRepo _LBAPlanRepo;
        public LedgerBudgetPlanController(ILBAPlanRepo LBAPlanRepo)
        {
            this._LBAPlanRepo = LBAPlanRepo;
        }
        // GET: LedgerBudgetPlan
        public ActionResult Index()
        {
            return PartialView();
        }
        public ActionResult FavBudgetPlanReport()
        {
            return PartialView();
        }

        public ActionResult CreateLedgerBudgetPlan(string plancode)
        {
        ...
        public ActionResult LedgerBudgetPlanSetup(string planCode)
        {
            ViewBag.planCode = planCode;
            return View();
        }
        ...
        {
            //SalesPlan result = this._LBAPlanRepo.GetPlanDetailValueByPlanCode(planCode);
        ...
        public JsonResult CopyPlan(FormCollection formData)
        {
            ...
            {
                copied = this._LBAPlanRepo.cloneSalesPlan(planCode, planName, branchs, divisions, remarks);
            }
            else
            {
                copied = false;
        ...
        public ActionResult LoadLedgerBudgetPlanSetupTreeListPartial(string startDate, string endDate, string timeFrameCode, string timeFrameName, string datetype)
        {
            return PartialView("~/Areas/NeoERP.Planning/Views/LedgerBudgetPlan/_LedgerBudgetPlanSetupTreeList.cshtml");
        }

        public string saveLedgerBudgetPlan(FormCollection fc)
        {
            ...
            foreach (var arr in parentArr)
            {
                ...
                var arrayValInt = Math.Truncate(Convert.ToDouble(decimalval));
                if (Math.Truncate(Convert.ToDouble(freqValueSum)) != arrayValInt)
                    continue;
            }
            if (parentArr.Count > 0)
                //return result = _iPlanSetup.SavePlan(parentArr, plan_Code, time_frame_code);
                return result = this._LBAPlanRepo.SaveLedgerBudgetPlan(parentArr, sp);
            else
                return "error";
        }
