...
namespace NeoERP.Planning.Controllers
{
    ...
    public class CollectionPlanController : Controller
    {
        ...
        public IPlan _plan;
        ICollectionPlanRepo _collectionPlanRepo;


        public CollectionPlanController(IPlanSetup iPlanSetup, ICollectionPlanRepo CollectionPlanRepo, IPlan PlanRepo)
        {
        ...
        public ActionResult FavCollectionPlanReport()
        {
            return PartialView();
        }
        public ActionResult CreateCollectionPlan(string plancode)
        {
            ...
            string employeCode = this._plan.getUsersEmployeeCode();
            ViewBag.EmployeeCode = employeCode;
            ViewBag.EMP_CODE = _collectionPlanRepo.GetEmployeeCodeByPlanCode(plancode);
            //ViewBag.CUSTOMER_CODE = _collectionPlanRepo.GetCustomerCodeByPlanCode(plancode);
            var model = this._iPlanSetup.GetPreferenceSetup("COLLECTION_PLAN");
            return PartialView(model);
        }
        public ActionResult CollectionFromMaterial(string plancode)
        {
            ViewBag.PlanCode = plancode;
            ViewBag.ProductSelectionLimit = System.Configuration.ConfigurationManager.AppSettings.Get("ProductSelectionLimit");
            string employeCode = this._plan.getUsersEmployeeCode();
        ...
        public ActionResult CollectionPlanSetup(string planCode)
        {
            ViewBag.planCode = planCode;
        ...
        {
            return PartialView("~/Areas/NeoERP.Planning/Views/CollectionPlan/_CollectionPlanSetupTreeList.cshtml");
        }
        public ActionResult LoadMaterialPlanGridPartial()
        {
            return PartialView("~/Areas/NeoERP.Planning/Views/CollectionPlan/_CollectionPlanSetupTreeList.cshtml");
        }
        public string SaveCollectionPlan(FormCollection fc)
        {
            ...
            CollectionPlan sp = new CollectionPlan()
            {
                ...
                REFERENCE_FLAG = fc["REFERENCE_FLAG"],
                REFERENCE_PLAN_CODE = fc["MATERIAL_PLAN_CODE"],
            ...
            if (parentArr.Count > 0)
                ...
                return result = this._collectionPlanRepo.SaveCollectionPlan(parentArr, sp);
            else
                return "error";
        }
        ...
        {
            var model = this._iPlanSetup.GetPreferenceSetup("COLLECTION_PLAN");
            PL_BRD_CLTN_PLAN result = this._collectionPlanRepo.GetPlanDetailValueByPlanCode(planCode);
            ViewBag.planCode = planCode;
            return PartialView(model);
        }
        [HttpPost]
        public JsonResult CopyPlan(FormCollection formData)
        {
            ...
            {
                copied = this._collectionPlanRepo.cloneCollectionPlan(planCode, planName, customers, employees, branchs, divisions, remarks);
