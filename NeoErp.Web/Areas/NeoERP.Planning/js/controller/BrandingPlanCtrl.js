planningModule.controller('BrandingPlanCtrl', function ($scope, planservice, $routeParams, $rootScope, $http) {
        ...
        $scope.pageName = "New Branding Plan";
            ...
            if (Isvalid) {
                ...
                if (itemLength < parseInt(productSelectionLimit)) {
                    if ($scope.param != '' && $scope.param != undefined && $scope.param != 'undefined') {
                        window.location.href = "/Planning/Home/Index#!Planning/BrandingPlanSetup/" + $scope.param;
                    } else {
                        window.location.href = "/Planning/Home/Index#!Planning/BrandingPlanSetup";
                    }
                }
                //if (itemLength < parseInt(productSelectionLimit)) {
                //    if ($scope.param != '' && $scope.param != undefined && $scope.param != 'undefined') {
        ...
        if (tempValue != undefined) {
            ...
            $.ajax({
                ...
                url: '/api/BrandingPlanApi/GetPlanDetailValueByPlanCode',
            ...
            if (tempValue != undefined) {
                setTimeout(function () {
                    ...
                    $.each(tempValue.checkeOnlyDataTemp, function (i, v) {
                        ...
                        var getitem = productTreeView.dataSource.get(itemCode);
                        var items = productTreeView.findByUid(getitem.uid);
                        var item = productTreeView.findByText(v.itemName);

                        if (items.length > 0) {
                            productTreeView.dataItem(items).set("checked", true);
                        }
                    });
                    ...
                    for (var i = 0; i < ret.selectedItemsList.length; i++) {
                        ...
                        if (i == (ret.selectedItemsList.length - 1)) {
                            $.each(ret.selectedItemsList, function (i, v) {
                                ...
                                if (items.length > 0) {
                                    if (v.GROUP_SKU_FLAG == 'Y') {
                                        productTreeView.dataItem(items).set("dirty", true);
