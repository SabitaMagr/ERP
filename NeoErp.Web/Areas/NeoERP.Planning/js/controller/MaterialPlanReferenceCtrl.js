planningModule.controller('MaterialPlanReferenceCtrl', function ($scope, planservice, $routeParams, $rootScope, $http) {
    ...
    $scope.pageName = "Reference Material Plan";
        ...
        if (Isvalid) {
            ...
            if (itemLength < parseInt(productSelectionLimit)) {
                if ($scope.param != '' && $scope.param != undefined && $scope.param != 'undefined') {
                    window.location.href = "/Planning/Home/Index#!Planning/MaterialPlanSetup/" + $scope.param;
                } else {
                    window.location.href = "/Planning/Home/Index#!Planning/MaterialPlanSetup";
                }
            }
            else {
    ...
    if (tempValue != undefined) {
        ...
        if (tempValue.MATERIAL_PLAN_CODE != "") {
            $("#ManualRadio").attr("checked", false);
            $("#fromMRPRadio").attr("checked", true);
            $("#ProductionPlanList").data("kendoDropDownList").value([tempValue.MATERIAL_PLAN_CODE]);
        ...
        showloader();
        $.ajax({
            url: '/api/MaterialPlanApi/GetPlanDetailValueByPlanCode',
            ...
            if (ret.REFERENCE_FLAG != "") {
                $("#ManualRadio").attr("checked", false);
                $("#fromMRPRadio").attr("checked", true);
                $("#MaterialPlanList").data("kendoDropDownList").value([ret.REFERENCE_PLAN_CODE]);
                $('input[name=Manual_MRP]:radio').trigger("change");
                $.when(productDeffer, tempValue_deffer).then(function () {
                    $(".k-checkbox").attr("disabled", true);
                });
