planningModule.controller('CollectionPlanCtrl', function ($scope, planservice, $routeParams, $rootScope, $http) {
    ...
    //    $('input[name=Manual_MRP]').trigger("click");
    //}
    ...
    $scope.customerFlag = '';
    $scope.divisionFlag = '';
    $scope.DivisionCodeOnChange = '';
    $rootScope.SelectedDivisionCodeOnChange = '';
    $scope.EmployeeCodeOnChange = '';
    $rootScope.SelectedEmployeeCodeOnChange = '';
    //$rootScope.BackDivisionCode = '';

    $scope.planName = planName;
    $scope.refOrManual = $routeParams.ref;
    ...
    function getFlagFromPreferenceSetup() {
        var url = window.location.protocol + "//" + window.location.host + "/api/PlanApi/GetCollectionPreferenceSetupFlag";
        var req = {
            method: 'GET',
            url: url,
        }
    ...
    $scope.pageName = "New Collection Plan";
    ...
    function IsFormValid() {
        ...
        //else if (IsCustomerSelected == "customer_product") {
        //    return false;
        //}
        else if (message != "") {
            return false;
        }
        else if ((employeeSelected == "" || employeeSelected == null) && loggedInUsers_employeeCode == "" && IsCustomerSelected != "customer_product") {
            return false;
        }
        else if (count < 1) {
            return false;
        ...
        else if (employeeSelected === "" || employeeSelected === null || employeeSelected === "null")
        {
            return false;
        }
        //else if (customerSelected === "" || customerSelected === null || customerSelected === "null")
        //{
        //    return false;
        //}
        else {
    ...
    $scope.gotoPlanSetup = function () {
        ...
        var manualRef = "?planName=" + planName + "&ref=" + ref;
        if (Isvalid) {
            var itemLength = 0
            ...
            if (itemLength < parseInt(productSelectionLimit)) {
                if ($scope.param != '' && $scope.param != undefined && $scope.param != 'undefined') {
                    if (planName != undefined && planName != "" && ref != undefined && ref != "")
                        window.location.href = "/Planning/Home/Index#!Planning/CollectionPlanSetup/" + $scope.param + manualRef;
                    else
                        window.location.href = "/Planning/Home/Index#!Planning/CollectionPlanSetup/" + $scope.param;
                } else {
                    if (planName != undefined && planName != "" && ref != undefined && ref != "")
                        window.location.href = "/Planning/Home/Index#!Planning/CollectionPlanSetup" + manualRef;
                    else
                        window.location.href = "/Planning/Home/Index#!Planning/CollectionPlanSetup";
                }
            }
            else {
    ...
    $scope.bindEmployeeOption = function () {
        ...
        $scope.employeeOptions = {
            ...
            //    if ($rootScope.BackEmployeeCode !== "" || $rootScope.BackEmployeeCode !== undefined) {
            //          var val = $rootScope.BackEmployeeCode;
    ...
    $scope.DivisionCodeOnChange = function (kendoEvent) {
        if (kendoEvent.sender.dataItem() !== undefined) {
            var value = $("#divisions").data("kendoDropDownList").value();
            $rootScope.SelectedDivisionCodeOnChange = value;
        }
    };
    $scope.EmployeeCodeOnChange = function (kendoEvent) {
        if (kendoEvent.sender.dataItem() !== undefined) {
            var value = $("#employees").data("kendoDropDownList").value();
            $rootScope.SelectedEmployeeCodeOnChange = value;
        }
    };
    ...
    if (tempValue != undefined) {
        ...
        setTimeout(function () {
            ...
            setTimeout(function () {
                ...
                if ($("#customers").data('kendoComboBox') != undefined)
                    $("#customers").data('kendoComboBox').dataSource.read();
            }, 1500);
            tempValue_deffer.resolve();
        }, 1000);
        $("#ManualRadio").attr("checked", true);
        if (tempValue.REFERENCE_FLAG != "" && tempValue.REFERENCE_FLAG != null && tempValue.REFERENCE_FLAG != "Manual") {
            $('input[name=Manual_MRP][value="' + tempValue.REFERENCE_FLAG + '"]:radio').prop('checked', true);
            var rff = tempValue.REFERENCE_FLAG;
            if (rff != "" && rff != null) {
                if (rff == "FROM_SALES")
                    rff = "Sales";
                else if (rff == "FROM_PRODUCTION")
                    rff = "Production";
                else if (rff == "FROM_MRP")
                    rff = "Material";
                $("#" + rff + "PlanList").data("kendoMultiSelect").value(tempValue.MATERIAL_PLAN_CODE);
                //$("#" + rff + "PlanList").data("kendoDropDownList").value([tempValue.MATERIAL_PLAN_CODE]);
            }
            $.when(productDeffer, tempValue_deffer).then(function () {
                $(".k-checkbox").attr("disabled", true);
            });
        }
        $('input[name=Manual_MRP]:radio').trigger("change");
        $scope.next = true;
        ...
        showloader();
        $.ajax({
            url: '/api/CollectionPlanApi/GetPlanDetailValueByPlanCode',
            ...
            $scope.planfor = ret.SALES_AMOUNT != null ? 'AMOUNT' : 'AMOUNT';
            $scope.customer_product_option = 'customer_product';//(ret.customerCode == null || ret.customerCode == '') ? 'product_only' : 'customer_product';
            if (ret.customerCode == null || ret.customerCode == '') {
                $scope.IsCustomerProduct = true;
            }
            $("#ManualRadio").attr("checked", true);
            if (ret.REFERENCE_FLAG != "" && ret.REFERENCE_FLAG != null) {
                $('input[name=Manual_MRP][value="FROM_' + ret.REFERENCE_FLAG + '"]:radio').prop('checked', true);
                var rff = ret.REFERENCE_FLAG;
                if (rff != "") {
                    if (rff == "SALES")
                        rff = "Sales";
                    else if (rff == "PRODUCTION")
                        rff = "Production";
                    else if (rff == "MRP")
                        rff = "Material";
                    //$("#" + rff + "PlanList").data("kendoDropDownList").value([ret.REFERENCE_PLAN_CODE]);
                    $("#" + rff + "PlanList").data("kendoMultiSelect").value([ret.REFERENCE_PLAN_CODE]);
                }
                $.when(productDeffer, tempValue_deffer).then(function () {
                    $(".k-checkbox").attr("disabled", true);
                });
