...
{
    public class ProcurementPlanRepo : IProcurementPlanRepo
    {
        private IDbContext _dbContext;
        private IWorkContext _workcontext;
        public ProcurementPlanRepo(IDbContext dbContext, IWorkContext workContext)
        {
        ...
        public string SaveProcurementPlan(List<savePlan> sv, ProcurementPlan sp)
        {
            try
            {
                ...
                // first insertion to PL_BRD_PRCMT_PLAN table
                string checkPlanQuery = $@"SELECT COUNT(*) FROM PL_BRD_PRCMT_PLAN WHERE PLAN_EDESC='{sp.PLAN_EDESC}' 
                ...
                {
                    string sales_plan_query = $@"INSERT INTO PL_BRD_PRCMT_PLAN(PLAN_CODE,PLAN_EDESC,PLAN_NDESC,EST_AMOUNT,TIME_FRAME_CODE,
                    ...
                    VALUES((SELECT COALESCE(MAX(PLAN_CODE)+1, MAX(PLAN_CODE) + 1, 1) FROM PL_BRD_PRCMT_PLAN),'{sp.PLAN_EDESC}','','{EST_AMOUNT}','{sp.TIME_FRAME_CODE}','{sp.CALENDAR_TYPE}',
                    ...
                    string fetchProcurementPlan = $@"SELECT PLAN_CODE FROM PL_BRD_PRCMT_PLAN WHERE PLAN_EDESC='{sp.PLAN_EDESC}' 
                AND TIME_FRAME_CODE='{sp.TIME_FRAME_CODE}' AND START_DATE=TO_DATE('{sp.START_DATE}','YYYY-Mon-DD') AND END_DATE=TO_DATE('{sp.END_DATE}','YYYY-Mon-DD')";
                ...
                else
                {
                    if (!string.IsNullOrEmpty(sp.PLAN_CODE) && sp.PLAN_CODE != "0")
                    {
                        ...
                        string updateQuery = $@"UPDATE PL_BRD_PRCMT_PLAN SET PLAN_EDESC = '{sp.PLAN_EDESC}' , REMARKS = '{sp.REMARKS}' 
                    ...
                    {
                        string plancode_query = $@"SELECT PLAN_CODE FROM PL_BRD_PRCMT_PLAN WHERE PLAN_EDESC='{sp.PLAN_EDESC}' 
                AND TIME_FRAME_CODE='{sp.TIME_FRAME_CODE}' AND START_DATE=TO_DATE('{sp.START_DATE}','YYYY-Mon-DD') AND END_DATE=TO_DATE('{sp.END_DATE}','YYYY-Mon-DD')";
                        planCode = this._dbContext.SqlQuery<int>(plancode_query).First();
                    }

                    string delete_detail_already_set = $@"DELETE FROM PL_BRD_PRCMT_PLAN_DTL WHERE PLAN_CODE='{planCode}'";
                ...
                foreach (var item in sv)
                {
                    ...
                    foreach (var freq in item.frequency)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                var sqlquery = $@"UPDATE PL_BRD_PRCMT_PLAN_DTL SET TARGET_VALUE='{freq.fvalue}',LAST_MODIFIED_BY='{userID}', LAST_MODIFIED_DATE=TO_DATE('{DateTime.Now:MM/dd/yyyy/ HH:mm:ss}', 'mm/dd/yyyy hh24:mi:ss') where PLAN_CODE='{planCode}' AND ITEM_CODE='{item.itemCode}' AND TIME_FRAME_CODE='{time_frame_code}' AND TIME_FRAME_VALUE='{freq.fname}'";
                            }
                        }
                    }
                    if (itteration <= (sv.Count() - remainderCount))
                    {
                        ...
                        {
                            string query = @"insert into PL_BRD_PRCMT_PLAN_DTL(PLAN_CODE,PLAN_DATE,PER_DAY_AMOUNT,ITEM_CODE,
                ...
                update_amout_quantity = $@"UPDATE PL_BRD_PRCMT_PLAN PSP SET PSP.EST_AMOUNT = (
                        SELECT ROUND(SUM(SPD.PER_DAY_AMOUNT),2) FROM PL_BRD_PRCMT_PLAN_DTL SPD, IP_ITEM_MASTER_SETUP IMS
                        WHERE 
        ...
        public List<ProcurementPlan> getAllProcurementPlans(ReportFiltersModel filters)
        {
            var companyCode = _workcontext.CurrentUserinformation.company_code;
            var userId = _workcontext.CurrentUserinformation.User_id;
            string query = $@"SELECT DISTINCT TO_CHAR(PL.PLAN_CODE) PLAN_CODE, PL.PLAN_EDESC, PL.PLAN_NDESC,
                             ...
                             PLD.DIVISION_CODE, TO_CHAR(PLD.BRANCH_CODE), DS.DIVISION_EDESC, FS.BRANCH_EDESC,PL.REFERENCE_FLAG FROM PL_BRD_PRCMT_PLAN PL, 
                             PL_BRD_PRCMT_PLAN_DTL PLD, FA_DIVISION_SETUP DS, FA_BRANCH_SETUP FS 
            ...
            List<ProcurementPlan> spList = new List<ProcurementPlan>();

            spList = this._dbContext.SqlQuery<ProcurementPlan>(query).ToList();
            return spList;
        }

        public List<ProcurementPlan> getAllProcurementPlans(string customercode, string employeecode, string divisioncode, string branchcode, string startdate, string enddate)
        {
            ...
            List<ProcurementPlan> mpList = new List<ProcurementPlan>();
            mpList = this._dbContext.SqlQuery<ProcurementPlan>(masterplan_query).ToList();
            if (mpList.Count > 0)
            {
                foreach (var item in mpList)
                {
                    spList.Add(new ProcurementPlan
                    {
        ...
        public bool deleteProcurementPlan(int planCode)
        {
            try
            {
                string deleteYes_salesPlan = $@"UPDATE PL_BRD_PRCMT_PLAN SET DELETED_FLAG='Y' WHERE PLAN_CODE='{planCode}'";
                string deleteYes_salesPlanDtl = $@"UPDATE PL_BRD_PRCMT_PLAN_DTL SET DELETED_FLAG='Y' WHERE PLAN_CODE='{planCode}'";
        ...
        {
            var company_code = this._workcontext.CurrentUserinformation.company_code;
            PL_BRD_PRCMT_PLAN ProcurementPlan = new PL_BRD_PRCMT_PLAN();
            string queryPlan = $@"SELECT 
                ...
                , REFERENCE_FLAG,REFERENCE_PLAN_CODE FROM PL_BRD_PRCMT_PLAN PSP
                WHERE PLAN_CODE= '{plancode}'";

            ProcurementPlan = this._dbContext.SqlQuery<PL_BRD_PRCMT_PLAN>(queryPlan).FirstOrDefault();

            List<ProcurementPlanDetail> salesPlanDetailList = new List<ProcurementPlanDetail>();
            string ProcurementPlanDetailQuery = $@"SELECT PLAN_CODE,PLAN_DATE PLAN_DATE,PER_DAY_AMOUNT PER_DAY_AMOUNT,
            ...
            salesPlanDetailList = this._dbContext.SqlQuery<ProcurementPlanDetail>(ProcurementPlanDetailQuery).ToList();

            List<ProcurementPlanItems> salesPlanItem = new List<ProcurementPlanItems>();
            string ProcurementPlanItemsQuery = $@"SELECT 
            ...
            salesPlanItem = this._dbContext.SqlQuery<ProcurementPlanItems>(ProcurementPlanItemsQuery).ToList();
        ...
        public bool cloneProcurementPlan(string planCode, string planName, string customers, string employees, string branchs, string divisions, string remarks)
        {
            try
            {
        ...
        public string CreateTemProcurementPlanReportTable(ReportFiltersModel model)
        {
            var companyCode = _workcontext.CurrentUserinformation.company_code;
            var response = "";
            var createTableQry = $@"CREATE TABLE TEMP_PL_PORCRUMENT_PLAN_REPORT AS  SELECT DISTINCT 
                                        ...
                                        SUBSTR(BS_DATE(PPD.PLAN_DATE), 6, 2) MONTHINT,
                                        TO_CHAR(substr(bs_date(PPD.PLAN_DATE), 0, 4)) as YEAR,   PPD.ITEM_CODE, IMS.ITEM_EDESC,                                
                                   SUM(PPD.PER_DAY_AMOUNT) PER_DAY_AMOUNT,PPD.DIVISION_CODE,DS.DIVISION_EDESC,FS.BRANCH_CODE, FS.BRANCH_EDESC,PPD.COMPANY_CODE     
                                   ...
                                   FA_BRANCH_SETUP FS
                                    ...
                                    SUBSTR(BS_DATE(PPD.PLAN_DATE), 6, 2),
                                    TO_CHAR(substr(bs_date(PPD.PLAN_DATE), 0, 4)) ,
                                    PPD.ITEM_CODE, IMS.ITEM_EDESC,FS.BRANCH_CODE,
                                   PPD.DIVISION_CODE, DS.DIVISION_EDESC,
                                   FS.BRANCH_EDESC,PPD.COMPANY_CODE";
            try
            {
                var tableExistsQry = $@"SELECT * FROM TEMP_PL_PORCRUMENT_PLAN_REPORT";
                var result = this._dbContext.SqlQuery<PlanReportModel>(tableExistsQry).ToList();
                if (result != null)
                {
                    var dropQry = $@"DROP TABLE TEMP_PL_PORCRUMENT_PLAN_REPORT";
        ...
        public List<ProcureFromMaterialModel> GetAllRawMaterialByMaterialCode(string pCode)
        {
            var company_code = _workcontext.CurrentUserinformation.company_code;
