...
{
    public class BrandingPlanRepo : IBrandingPlanRepo
    {
        private IDbContext _dbContext;
        private IWorkContext _workcontext;
        public BrandingPlanRepo(IDbContext dbContext, IWorkContext workContext)
        {
        ...
        #region Branding Plan
        public string SaveBrandingPlan(List<savePlan> sv, BrandingPlan sp)
        {
            try
            {
                ...
                // first insertion to PL_BRD_ACTIVITY_PLAN table
                string checkPlanQuery = $@"SELECT COUNT(*) FROM PL_BRD_ACTIVITY_PLAN WHERE PLAN_EDESC='{sp.PLAN_EDESC}' 
                ...
                {
                    string sales_plan_query = $@"INSERT INTO PL_BRD_ACTIVITY_PLAN(PLAN_CODE,PLAN_EDESC,PLAN_NDESC,EST_AMOUNT,TIME_FRAME_CODE,
                    CALENDAR_TYPE,START_DATE,END_DATE,REMARKS,COMPANY_CODE,BRANCH_CODE,CREATED_BY,CREATED_DATE,
                    LAST_MODIFIED_BY,LAST_MODIFIED_DATE,
                    DELETED_FLAG)
                    VALUES((SELECT COALESCE(MAX(PLAN_CODE)+1, MAX(PLAN_CODE) + 1, 1) FROM PL_BRD_ACTIVITY_PLAN),'{sp.PLAN_EDESC}','','{EST_AMOUNT}','{sp.TIME_FRAME_CODE}','{sp.CALENDAR_TYPE}',
                    ...
                    string fetchBrandingPlan = $@"SELECT PLAN_CODE FROM PL_BRD_ACTIVITY_PLAN WHERE PLAN_EDESC='{sp.PLAN_EDESC}' 
                AND TIME_FRAME_CODE='{sp.TIME_FRAME_CODE}' AND START_DATE=TO_DATE('{sp.START_DATE}','YYYY-Mon-DD') AND END_DATE=TO_DATE('{sp.END_DATE}','YYYY-Mon-DD')";
                    planCode = this._dbContext.SqlQuery<int>(fetchBrandingPlan).First();
                ...
                else
                {
                    if (!string.IsNullOrEmpty(sp.PLAN_CODE) && sp.PLAN_CODE != "0")
                    {
                        ...
                        string updateQuery = $@"UPDATE PL_BRD_ACTIVITY_PLAN SET PLAN_EDESC = '{sp.PLAN_EDESC}' , REMARKS = '{sp.REMARKS}' 
                    ...
                    {
                        string plancode_query = $@"SELECT PLAN_CODE FROM PL_BRD_ACTIVITY_PLAN WHERE PLAN_EDESC='{sp.PLAN_EDESC}' 
                AND TIME_FRAME_CODE='{sp.TIME_FRAME_CODE}' AND START_DATE=TO_DATE('{sp.START_DATE}','YYYY-Mon-DD') AND END_DATE=TO_DATE('{sp.END_DATE}','YYYY-Mon-DD')";
                        planCode = this._dbContext.SqlQuery<int>(plancode_query).First();
                    }

                    string delete_detail_already_set = $@"DELETE FROM PL_BRD_ACTIVITY_PLAN_DTL WHERE PLAN_CODE='{planCode}'";
                ...
                foreach (var item in sv)
                {
                    ...
                    foreach (var freq in item.frequency)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                var sqlquery = $@"UPDATE PL_BRD_ACTIVITY_PLAN_DTL SET TARGET_VALUE='{freq.fvalue}',LAST_MODIFIED_BY='{userID}', LAST_MODIFIED_DATE=TO_DATE('{DateTime.Now:MM/dd/yyyy/ HH:mm:ss}', 'mm/dd/yyyy hh24:mi:ss') where PLAN_CODE='{planCode}' AND ACTIVITY_CODE='{item.itemCode}' AND TIME_FRAME_CODE='{time_frame_code}' AND TIME_FRAME_VALUE='{freq.fname}'";
                            }
                        }
                    }
                    if (itteration <= (sv.Count() - remainderCount))
                    {
                        ...
                        {
                            string query = @"insert into PL_BRD_ACTIVITY_PLAN_DTL(PLAN_CODE,PLAN_DATE,PER_DAY_AMOUNT,ACTIVITY_CODE,
                ...
                update_amout_quantity = $@"UPDATE PL_BRD_ACTIVITY_PLAN PSP SET PSP.EST_AMOUNT = (
                      SELECT ROUND(SUM(SPD.PER_DAY_AMOUNT),2) FROM PL_BRD_ACTIVITY_PLAN_DTL SPD, BRD_ACTIVITY IMS
                        WHERE 
                        IMS.GROUP_ACTIVITY_FLAG ='N'
                        AND SPD.COMPANY_CODE = IMS.COMPANY_CODE
                        AND SPD.ACTIVITY_CODE = IMS.ACTIVITY_CODE
        ...
        public List<BrandingPlan> getAllBrandingPlans(ReportFiltersModel filters)
        {
            var companyCode = _workcontext.CurrentUserinformation.company_code;
            var userId = _workcontext.CurrentUserinformation.User_id;
            string query = $@"SELECT DISTINCT TO_CHAR(PL.PLAN_CODE) PLAN_CODE, PL.PLAN_EDESC, PL.PLAN_NDESC,
                             ...
                             PLD.DIVISION_CODE, TO_CHAR(PLD.BRANCH_CODE), DS.DIVISION_EDESC, FS.BRANCH_EDESC FROM PL_BRD_ACTIVITY_PLAN PL, 
                             PL_BRD_ACTIVITY_PLAN_DTL PLD, FA_DIVISION_SETUP DS, FA_BRANCH_SETUP FS 
            ...
            List<BrandingPlan> spList = new List<BrandingPlan>();

            spList = this._dbContext.SqlQuery<BrandingPlan>(query).ToList();
            return spList;
        }

        public List<BrandingPlan> getAllBrandingPlans(string customercode, string employeecode, string divisioncode, string branchcode, string startdate, string enddate)
        {
            ...
            string query = $@"SELECT DISTINCT TO_CHAR(PL.PLAN_CODE), TO_CHAR(PL.PLAN_CODE)||'_SP' PLAN_CODE, PL.PLAN_EDESC, PL.PLAN_NDESC,
                            ...
                            FS.BRANCH_EDESC
                            FROM PL_BRD_ACTIVITY_PLAN PL, PL_BRD_ACTIVITY_PLAN_DTL PLD, SA_CUSTOMER_SETUP CS , FA_DIVISION_SETUP DS, HR_EMPLOYEE_SETUP ES, FA_BRANCH_SETUP FS
            ...
            string masterplan_query = $@"SELECT DISTINCT TO_CHAR(MSP.MASTER_PLAN_CODE)||'_MP' PLAN_CODE,
                                    ...
                                    FROM PL_MASTER_SALES_PLAN MSP ,PL_BRD_ACTIVITY_PLAN_MAP SPM
            ...
            List<BrandingPlan> mpList = new List<BrandingPlan>();
            mpList = this._dbContext.SqlQuery<BrandingPlan>(masterplan_query).ToList();
            if (mpList.Count > 0)
            {
                foreach (var item in mpList)
                {
                    spList.Add(new BrandingPlan
                    {
        ...
        public bool deleteBrandingPlan(int planCode)
        {
            try
            {
                string deleteYes_salesPlan = $@"UPDATE PL_BRD_ACTIVITY_PLAN SET DELETED_FLAG='Y' WHERE PLAN_CODE='{planCode}'";
                string deleteYes_salesPlanDtl = $@"UPDATE PL_BRD_ACTIVITY_PLAN_DTL SET DELETED_FLAG='Y' WHERE PLAN_CODE='{planCode}'";
        ...
        //public PL_BRD_PLAN GetPlanDetailValueByPlanCode(int plancode)
        //{
        //    var company_code = this._workcontext.CurrentUserinformation.company_code;
        //    PL_BRD_PLAN BrandingPlan = new PL_BRD_PLAN();
        ...
        //        FROM PL_BRD_ACTIVITY_PLAN PSP
        //        WHERE PLAN_CODE= '{plancode}'";

        //    BrandingPlan = this._dbContext.SqlQuery<PL_BRD_PLAN>(queryPlan).FirstOrDefault();

        //    List<BrandingPlanDetail> salesPlanDetailList = new List<BrandingPlanDetail>();
        //    string BrandingPlanDetailQuery = $@"SELECT PLAN_CODE,PLAN_DATE PLAN_DATE,PER_DAY_AMOUNT PER_DAY_AMOUNT,
        //        ACTIVITY_CODE ITEM_CODE,DIVISION_CODE,REMARKS,COMPANY_CODE,BRANCH_CODE,CREATED_BY,CREATED_DATE CREATED_DATE, FREQUENCY_JSON
        //        FROM PL_BRD_ACTIVITY_PLAN_DTL WHERE PLAN_CODE = '{plancode}' AND DELETED_FLAG='N'";
        //    salesPlanDetailList = this._dbContext.SqlQuery<BrandingPlanDetail>(BrandingPlanDetailQuery).ToList();

        //    List<BrandingPlanItems> salesPlanItem = new List<BrandingPlanItems>();
        //    string BrandingPlanItemsQuery = $@"        SELECT 
        //        DISTINCT TO_CHAR(PSPD.ACTIVITY_CODE) ITEM_CODE,INITCAP(IMS.ACTIVITY_EDESC) ITEM_EDESC,TO_CHAR(IMS.MASTER_ACTIVITY_CODE) MASTER_ITEM_CODE,TO_CHAR(IMS.PARENT_ACTIVITY_CODE) PRE_ITEM_CODE,
        //        TO_CHAR(IMS.GROUP_ACTIVITY_FLAG) GROUP_SKU_FLAG
        //        FROM PL_BRD_ACTIVITY_PLAN_DTL PSPD,BRD_ACTIVITY IMS WHERE PSPD.PLAN_CODE='01' AND PSPD.DELETED_FLAG='N' AND IMS.DELETED_FLAG='N' AND IMS.ACTIVITY_CODE=PSPD.ACTIVITY_CODE
        //        ORDER BY IMS.PARENT_ACTIVITY_CODE";
        //    salesPlanItem = this._dbContext.SqlQuery<BrandingPlanItems>(BrandingPlanItemsQuery).ToList();

        //    BrandingPlan.selectedItemsList = salesPlanItem;
        //    BrandingPlan.salesPlanDetail = salesPlanDetailList;
        ...
        public COAPlanSetupDetailViewModal GetBudgetPlanDetailValueByPlanCode(int plancode)
        {
            ...
            string queryCoaPlanDtlList = $@"SELECT PLAN_CODE PLAN_CODE,PLAN_DATE PLAN_DATE,PER_DAY_AMOUNT PER_DAY_AMOUNT,
                ACTIVITY_CODE ACC_CODE,DIVISION_CODE,REMARKS,COMPANY_CODE,BRANCH_CODE,CREATED_BY,CREATED_DATE CREATED_DATE, FREQUENCY_JSON
                FROM PL_BRD_ACTIVITY_PLAN_DTL WHERE PLAN_CODE = '{plancode}' AND DELETED_FLAG='N'";
            ...
            string queryCoaPlanItem = $@"SELECT 
                DISTINCT TO_CHAR(PSPD.ACTIVITY_CODE) ITEM_CODE,INITCAP(IMS.ACTIVITY_EDESC) ITEM_EDESC,TO_CHAR(IMS.MASTER_ACTIVITY_CODE) MASTER_ITEM_CODE,TO_CHAR(IMS.PARENT_ACTIVITY_CODE) PRE_ITEM_CODE,
                TO_CHAR(IMS.GROUP_ACTIVITY_FLAG) GROUP_SKU_FLAG
                FROM PL_BRD_ACTIVITY_PLAN_DTL PSPD,BRD_ACTIVITY IMS WHERE PSPD.PLAN_CODE='{plancode}' AND PSPD.DELETED_FLAG='N' AND IMS.DELETED_FLAG='N' AND IMS.ACTIVITY_CODE=PSPD.ACTIVITY_CODE
                ORDER BY IMS.PARENT_ACTIVITY_CODE";
        ...
        public List<PlanSalesRefrenceModel> getSalesItemDataForRefrence(string itemList, string startDate, string endDate, string customerCode, string divisionCode, string branchCode, string dateFormat, string frequency)
        {
            ...
            try
            {
                ...
                if (dateFormat.ToUpper() == "BS" || dateFormat.ToUpper() == "LOC")
                {
                    ...
                    {
                        refrenceQuery = $@" SELECT  'freqItemNum_'||SI.ACTIVITY_CODE ||'_WEEK_' || NEPALI_WEEKS.WEEKS ||'_'||  TO_CHAR(SUBSTR(BS_DATE(SI.SALES_DATE),0,4) )  COLNAME, SI.ACTIVITY_CODE,
                                         IMS.ITEM_EDESC,
                                         SI.CUSTOMER_CODE,
                                         SI.DIVISION_CODE,
                                         SI.BRANCH_CODE,
                                ...
                                WHERE IMS.ACTIVITY_CODE IN ({itemList}) {optionalCondition} 
                                AND SI.ACTIVITY_CODE = IMS.ACTIVITY_CODE
                    ...
                    {
                        refrenceQuery = $@"SELECT 'freqItemNum_'||SI.ACTIVITY_CODE ||'_' ||  UPPER(fn_bs_month(SUBSTR(BS_DATE(SI.SALES_DATE),6,2))) ||'_'|| SUBSTR(BS_DATE(SI.SALES_DATE),0,4) COLNAME ,
                                     IMS.ITEM_EDESC,
                                     SI.CUSTOMER_CODE,
                                     SI.DIVISION_CODE,
                                     SI.BRANCH_CODE,
                ...
                else if (dateFormat.ToUpper() == "AD" || dateFormat.ToUpper() == "ENG")
                {
                    if (frequency.ToLower() == "week")
                    {
                        refrenceQuery = $@"SELECT 'freqItemNum_'||SI.ACTIVITY_CODE ||'_WEEK_' ||  TO_CHAR(SI.SALES_DATE,'IW') ||'_'||  TO_CHAR(SI.SALES_DATE,'YYYY') COLNAME, SI.ACTIVITY_CODE,
                                       IMS.ITEM_EDESC,
                                     SI.CUSTOMER_CODE,
                                     SI.DIVISION_CODE,
                                     SI.BRANCH_CODE,
                    ...
                    {
                        refrenceQuery = $@"SELECT  'freqItemNum_'||SI.ACTIVITY_CODE ||'_' ||  TO_CHAR(SI.SALES_DATE,'MON') ||'_'||  TO_CHAR(SI.SALES_DATE,'YYYY') COLNAME ,
                                     IMS.ITEM_EDESC,
                                     SI.CUSTOMER_CODE,
                                     SI.DIVISION_CODE,
                                     SI.BRANCH_CODE,
        ...
        public List<PlanSalesRefrenceModel> getBrandingItemDataForStock(string itemList, string startDate, string endDate, string customerCode, string divisionCode, string branchCode, string dateFormat, string frequency)
        {
            var userNo = _workcontext.CurrentUserinformation.User_id;
            var result = new List<PlanSalesRefrenceModel>();
            try
            {
                ...
                refrenceQuery = $@"SELECT 'freqItemNum_'||SI.ACTIVITY_CODE ||'_' ||  UPPER(fn_bs_month(SUBSTR(BS_DATE(SI.VOUCHER_DATE),6,2))) ||'_'|| SUBSTR(BS_DATE(SI.VOUCHER_DATE),0,4) COLNAME ,
        ...
        public bool cloneBrandingPlan(string planCode, string planName, string customers, string employees, string branchs, string divisions, string remarks)
        {
            try
            {
                remarks = remarks.Replace("\"", " ");
                string existingPlanNameQuery = $@"SELECT PLAN_EDESC FROM PL_BRD_ACTIVITY_PLAN WHERE PLAN_CODE='{planCode}'";
                ...
                if (existingPlanName != null)
                {
                    ...
                    string maxPlanCodeQuery = "SELECT COALESCE(MAX(PLAN_CODE)+1, MAX(PLAN_CODE) + 1, 1) FROM PL_BRD_ACTIVITY_PLAN";
                    int maxPlanCode = this._dbContext.SqlQuery<int>(maxPlanCodeQuery).First();

                    string copySalesPlanQuery = $@"INSERT INTO PL_BRD_ACTIVITY_PLAN(PLAN_CODE,PLAN_EDESC,PLAN_NDESC,EST_AMOUNT,TIME_FRAME_CODE,
                    CALENDAR_TYPE,START_DATE,END_DATE,REMARKS,COMPANY_CODE,BRANCH_CODE,CREATED_BY,CREATED_DATE,
                    DELETED_FLAG)
                    SELECT '{maxPlanCode}','{copyPlanName}', PLAN_NDESC,EST_AMOUNT,TIME_FRAME_CODE,
                    CALENDAR_TYPE,START_DATE,END_DATE,'{remarks}',COMPANY_CODE,BRANCH_CODE,CREATED_BY,CREATED_DATE,
                    DELETED_FLAG FROM PL_BRD_ACTIVITY_PLAN WHERE PLAN_CODE='{planCode}'";
                    var insertCopiedPlan = this._dbContext.ExecuteSqlCommand(copySalesPlanQuery);

                    int copiedPlanCode = maxPlanCode;
                    if (copiedPlanCode > 0)
                    {
                        string copyinto_plandtl = $@"INSERT INTO PL_BRD_ACTIVITY_PLAN_DTL
                                (PLAN_CODE,PLAN_DATE,PER_DAY_AMOUNT,ACTIVITY_CODE,
                                ...
                                SELECT '{copiedPlanCode}', PLAN_DATE,PER_DAY_AMOUNT,ACTIVITY_CODE,
                               '{divisions}',REMARKS,
                                COMPANY_CODE,'{branchs}',CREATED_BY,CREATED_DATE,LAST_MODIFIED_BY,LAST_MODIFIED_DATE,DELETED_FLAG,FREQUENCY_JSON FROM PL_BRD_ACTIVITY_PLAN_DTL WHERE PLAN_CODE='{planCode}'";
        ...
        #endregion
        #region Treeview
        public List<ProductSetupModel> getAllBrandingActivityWithChildItem()
        {
            var company_code = _workcontext.CurrentUserinformation.company_code;
            string Query = $@"SELECT Distinct LEVEL, 
                            INITCAP(ACTIVITY_EDESC) AS ItemName,
                            ACTIVITY_CODE AS ItemCode,
                            ACTIVITY_CODE AS MasterItemCode, 
                            TO_CHAR(MASTER_ACTIVITY_CODE) AS PreItemCode, 
                            GROUP_ACTIVITY_FLAG AS GroupFlag
                            FROM BRD_ACTIVITY 
                            WHERE DELETED_FLAG = 'N' 
                            AND COMPANY_CODE = '{company_code}' 
                            START WITH MASTER_ACTIVITY_CODE IS NULL
                            CONNECT BY PRIOR ACTIVITY_CODE = MASTER_ACTIVITY_CODE";

            var BrandingActivityListNodes = _dbContext.SqlQuery<ProductSetupModel>(Query).ToList();
            return BrandingActivityListNodes;
        }
        
        public List<ProductTree> ChartOfAccountList()
        {
            List<ProductTree> list = new List<ProductTree>();
            string query = $@"SELECT ACTIVITY_CODE ITEMCODE, ACTIVITY_EDESC ITEMNAME  FROM BRD_ACTIVITY WHERE DELETED_FLAG='N' AND COMPANY_CODE='{_workcontext.CurrentUserinformation.company_code}'";
